...
const PropertyDetailsScreen = () => {
  ...
  const swiperImages = [images.japan, images.map, images.newYork];
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-white items-center justify-center">
        ...
        <Text className="text-black-200 font-rubik mt-4">Loading property...</Text>
      </View>
    );
  }
  ...
  if (error || !property) {
    return (
      <View className="flex-1 bg-white items-center justify-center px-5">
        <Text className="text-black-300 font-rubik-semibold text-lg mb-2">
          Property Not Found
        </Text>
        <Text className="text-black-200 font-rubik text-center mb-5">
          {error ? "Unable to load property details." : "This property doesn't exist."}
        </Text>
        <TouchableOpacity
          onPress={() => router.back()}
          className="bg-primary-300 px-6 py-3 rounded-full"
        >
          <Text className="text-white font-rubik-semibold">Go Back</Text>
        </TouchableOpacity>
      </View>
    );
  }
          ...
          <View className="px-5">
            <View className="py-5 w-full border-b border-primary-200 flex flex-col gap-2 items-start">
              ...
              <View className="flex flex-row items-center gap-4">
                <TouchableOpacity className="bg-primary-100 rounded-full px-2 py-1">
                  <Text className="uppercase text-primary-300 font-rubik-semibold text-[10px]">
                ...
                <View className="flex flex-row items-center">
                  ...
                  <Text className="text-black-200 font-rubik-medium text-sm">
                    {property.rating || "4.8"} ({property.reviews || "1,275"} reviews)
                  </Text>
                </View>
              </View>
              <View className="mt-4 flex flex-row gap-4 items-center flex-wrap">
                <View className="flex flex-row gap-2 items-center">
                  ...
                  <Text className="text-black-300 font-rubik-medium text-sm">
                ...
                <View className="flex flex-row gap-3 items-center">
                  ...
                  <View>
                    <Text className="text-black-300 text-lg font-rubik-semibold">
                      {property.agent?.name || "Natasya Wilodra"}
                    </Text>
                    <Text className="font-rubik-medium text-sm text-black-200">
                      {property.agent?.email || "Owner"}
                    </Text>
                  </View>
                </View>
                <View className="flex flex-row gap-3 items-center">
            ...
            <View className="mb-5 flex flex-col gap-2 items-start">
              ...
              <View className="w-full flex flex-row justify-start gap-3 items-center flex-wrap">
                {facilities.slice(0, 5).map((item, idx) => (
            ...
            <View className="mb-5 flex flex-col gap-2 items-start">
              ...
              <View className="w-full flex flex-row gap-3 justify-between h-40">
                <Image
                  className="h-full rounded-2xl flex-1"
                  source={property.gallery?.[0] ? { uri: property.gallery[0] } : images.newYork}
                  resizeMode="cover"
                />
                <Image
                  className="h-full rounded-2xl flex-1"
                  source={property.gallery?.[1] ? { uri: property.gallery[1] } : images.newYork}
                  resizeMode="cover"
                />
                <View className="h-full flex-1 relative rounded-2xl overflow-hidden flex items-center justify-center">
                  <Image
                    className="size-full rounded-2xl"
                    source={property.gallery?.[2] ? { uri: property.gallery[2] } : images.newYork}
                    resizeMode="cover"
                  />
                  <Image
                    source={images.cardGradient}
                    className="absolute size-full rounded-2xl bottom-0"
                  />
                  <Text className="text-white absolute text-xl font-rubik-bold">
                    {property.gallery?.length ? `${property.gallery.length - 2}+` : "20+"}
                  </Text>
                </View>
              </View>
            </View>
            <View className="mb-5 flex flex-col gap-2 items-start">
              ...
              <View className="flex flex-row justify-start gap-2 items-center">
                ...
                <Text className="text-black-200 font-rubik-medium">
                  {property.address || "Grand City St. 100, New York, United States"}
                </Text>
              </View>
              <Image 
                source={images.map}
            ...
            <View className="w-full flex gap-2 mb-7">
              <View className="flex flex-row gap-2 items-center">
                ...
                <Text className="text-black-300 text-base font-rubik-semibold">
                  Charolette Hanlin
                </Text>
              </View>
          ...
          <View className="rounded-2xl pt-3 pb-5 border-[1px] border-primary-200 px-6">
            <View className="flex flex-row items-center justify-between gap-10">
              <View className="flex items-start">
                <Text className="uppercase text-black-200 font-rubik-medium text-xs">
                  Price
                </Text>
                <Text className="text-primary-300 text-2xl font-rubik-semibold mt-1">
                  ${property.price || "17821"}
                </Text>
              </View>
